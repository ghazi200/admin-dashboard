/**
 * Migration Script: Create Messaging System Tables
 * 
 * Creates tables for in-app messaging:
 * - conversations
...
// Load .env - read DATABASE_URL directly from file to ensure we get the correct one
...
if (fs.existsSync(envPath)) {
  ...
  if (match) {
    ...
    // Parse to show database name
    try {
      const url = new URL(databaseUrl);
      console.log(`🔗 DATABASE_URL points to database: ${url.pathname.slice(1)}`);
    } catch (e) {
...
// Also load .env normally for other variables
require('dotenv').config({ path: envPath, override: true });

// Use the directly read DATABASE_URL, or fall back to process.env
...
// Database connection - use the DATABASE_URL we read
...
(async () => {
  try {
    ...
    // Verify we're connected to the correct database (abe_guard or abe-guard)
    const [dbInfo] = await sequelize.query('SELECT current_database() as db_name');
    const dbName = dbInfo[0]?.db_name;
    console.log(`📊 Connected to database: ${dbName}`);

    if (dbName === 'abe_guard' || dbName === 'abe-guard' || dbName.includes('abe_guard') || dbName.includes('abe-guard')) {
      console.log('✅ Correct database (abe_guard/abe-guard)\n');
    } else {
      console.log(`⚠️  WARNING: Connected to "${dbName}" instead of "abe_guard" or "abe-guard"`);
